...
void updateUpgradeScreen(UpgradeScreen *up, Vector2 mousePos) {
  ...
  case OPENING:
    ...
    if (up->currentOpening < 3) {
      ...
      spawnParticles(
          &upPS,
          (Vector2){up->upgradeRecs[i].x + up->upgradeRecs[i].width / 2,
                    up->upgradeRecs[i].y + 25},
          GetRandomValue(1, 3) * 0.15, WHITE,
      ...
      spawnParticles(
          &upPS,
          (Vector2){up->upgradeRecs[i].x + up->upgradeRecs[i].width / 2,
                    up->upgradeRecs[i].y + up->upgradeRecs[i].height - 25},
          GetRandomValue(1, 3) * 0.15, WHITE,
          (Vector2){GetRandomValue(-15, 15), GetRandomValue(5, 15)},
          GetRandomValue(5, 10));
...
void drawUpgrades(UpgradeScreen *up, Font font) {
  ...
  if (up->state != NOT_ACTIVE) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      DrawTexturePro(
          upgradeTextures[up->upgrade[i]], (Rectangle){0, 0, 33, 58},
          up->upgradeRecs[i], (Vector2){0, 0}, 0.0f,
          Fade((up->isHovering[i] ? WHITE : WHITE), up->openingProgress[i]));
      DrawRectangleLinesEx(
          up->upgradeRecs[i], borderThickness,
          Fade(up->isHovering[i] ? (Color){44, 232, 245} : WHITE,
               up->openingProgress[i]));
    }
  }
}
